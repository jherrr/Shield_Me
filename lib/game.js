(function () {
  ...
  var Game = ShieldME.Game = function () {
    this.asteroids = [];
    this.bullets = [];
    this.ships = [];
  ...
  Game.prototype.add = function (object) {
    if (object instanceof ShieldME.Asteroid) {
      this.asteroids.push(object);
    } else if (object instanceof ShieldME.Bullet) {
      this.bullets.push(object);
    } else if (object instanceof ShieldME.Ship) {
      this.ships.push(object);
    } else if (object instanceof ShieldME.Trail) {
      this.trail = object;
    } else {
      throw "wtf?";
    }
  ...
  Game.prototype.addShieldME = function () {
    for (var i = 0; i < Game.NUM_ASTEROIDS; i++) {
      this.add(new ShieldME.Asteroid({ game: this }));
    }
  };

  Game.prototype.addShip = function () {
    var ship = new ShieldME.Ship({
  ...
  Game.prototype.remove = function (object) {
    if (object instanceof ShieldME.Bullet) {
      this.bullets.splice(this.bullets.indexOf(object), 1);
    } else if (object instanceof ShieldME.Asteroid) {
      var idx = this.asteroids.indexOf(object);
      this.asteroids[idx] = new ShieldME.Asteroid({ game: this });
    } else if (object instanceof ShieldME.Ship) {
